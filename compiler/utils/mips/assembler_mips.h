...
class MipsAssembler FINAL : public Assembler {
 public:
  ...
  void EmitSlowPaths() OVERRIDE;

  // CFI helpers.
  void DebugFrameImplicitlyAdvancePC() OVERRIDE;
  void FinalizeDebugFramePCs();
 ...
 private:
  ...
  // CFI PC advances.
  typedef struct {
    size_t pos_;  // Postion in writer's buffer.
    size_t pc_;   // Old PC (can change on branch promotion from short to long).
  } PcAdvance;
  std::vector<PcAdvance> pc_advances_;
