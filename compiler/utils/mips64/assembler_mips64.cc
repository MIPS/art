...
void Mips64Assembler::EmitSlowPaths() {
...
void Mips64Assembler::DebugFrameImplicitlyAdvancePC() {
  CHECK_EQ(cfi_.GetCurrentPC(), 0);
  pc_advances_.push_back({cfi_.GetOpcodes().size(), CodeSize()});
}

void Mips64Assembler::FinalizeDebugFramePCs() {
  // This code is based on quick compiler's LazyDebugFrameOpCodeWriter::Patch().
  CHECK_EQ(cfi_.GetCurrentPC(), 0);
  std::vector<uint8_t>& opcodes = cfi_.GetOpcodes();
  // Move our data buffer to temporary variable.
  std::vector<uint8_t> old_opcodes(opcodes.get_allocator());
  old_opcodes.swap(opcodes);
  // Refill our data buffer with patched opcodes.
  opcodes.reserve(old_opcodes.size() + pc_advances_.size() + 4);
  size_t pos = 0;
  for (auto advance : pc_advances_) {
    CHECK_GE(advance.pos_, pos);
    // Copy old data up to the point when advance was issued.
    opcodes.insert(opcodes.end(), old_opcodes.begin() + pos, old_opcodes.begin() + advance.pos_);
    pos = advance.pos_;
    uint32_t new_pc = GetAdjustedPosition(advance.pc_);
    // Insert the advance command with its final offset.
    cfi_.AdvancePC(new_pc);
  }
  // Copy the final segment.
  opcodes.insert(opcodes.end(), old_opcodes.begin() + pos, old_opcodes.end());
  cfi_.AdvancePC(GetAdjustedPosition(CodeSize()));
}

void Mips64Assembler::FinalizeInstructions(const MemoryRegion& region) {
  EmitBranches();
  FinalizeDebugFramePCs();
...
void Mips64Assembler::CallFromThread64(ThreadOffset<8> offset ATTRIBUTE_UNUSED,
                                       ManagedRegister mscratch ATTRIBUTE_UNUSED) {
  UNIMPLEMENTED(FATAL) << "No MIPS64 implementation";
}
