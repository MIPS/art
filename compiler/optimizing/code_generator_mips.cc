...
void CodeGeneratorMIPS::Finalize(CodeAllocator* allocator) {
  ...
  // Adjust native pc offsets in PC info.
  for (size_t i = 0, num = pc_infos_.Size(); i != num; ++i) {
    auto& info = pc_infos_.GetRawStorage()[i];
    uint32_t old_position = info.native_pc;
    uint32_t new_position = __ GetAdjustedPosition(old_position);
    DCHECK_GE(new_position, old_position);
    info.native_pc = new_position;
  }
...
void ParallelMoveResolverMIPS::EmitSwap(size_t index) {
  ...
  if (loc1.IsRegister() && loc2.IsRegister()) {
    ...
    Exchange(loc1.GetStackIndex(), loc2.GetStackIndex(), /* double slot */ false);
  } else if (loc1.IsDoubleStackSlot() && loc2.IsDoubleStackSlot()) {
    Exchange(loc1.GetStackIndex(), loc2.GetStackIndex(), /* double slot */ true);
  } else {
...
void CodeGeneratorMIPS::MoveLocation(Location destination,
  ...
  if (source.IsConstant()) {
    MoveConst(destination, source.GetConstant());
  } else {
    if (Primitive::Is64BitType(type)) {
      Move64(destination, source);
    } else {
      Move32(destination, source);
    }
  }
}
...
void CodeGeneratorMIPS::MoveConst(Location destination, HConstant* c) {
...
void CodeGeneratorMIPS::Move(HInstruction* instruction,
  ...
  if (locations != nullptr && locations->Out().Equals(location)) {
    ...
    MoveConst(location, instruction->AsConstant());
...
void LocationsBuilderMIPS::HandleShift(HBinaryOperation* instr) {
  ...
  Location::OutputOverlap overlap = Location::kOutputOverlap;
  switch (type) {
    case Primitive::kPrimInt:
      overlap = Location::kNoOutputOverlap;
  ...
  switch (in_type) {
    case Primitive::kPrimLong: {
      ...
      if (use_imm) {
        ...
        if (value == 0) {
          // Special case for 0.
          __ Sra(TMP, lhs_high, 31);
          __ Or(AT, lhs_high, lhs_low);
          __ Sltu(AT, ZERO, AT);
          __ Or(res, TMP, AT);
        } else {
          // The algorithm for non-zero constants is the same as in the non-constant case below,
          // except for special handling of 0 and 0xFFFFFFFF of the constant halves.
          uint32_t low = Low32Bits(value);
          uint32_t high = High32Bits(value);
          Register reg = ZERO;
          if (high != 0) {
            reg = AT;
            __ LoadConst32(reg, high);
          }
          __ Slt(TMP, lhs_high, reg);
          __ Slt(AT, reg, lhs_high);
          __ Subu(res, AT, TMP);
          __ Bnez(res, &done);
          if (low == 0) {
            __ Sltu(res, ZERO, lhs_low);
          } else if (low == 0xFFFFFFFF) {
            __ Sltiu(res, lhs_low, low);
            __ Subu(res, ZERO, res);
          } else {
            __ LoadConst32(AT, low);
            __ Sltu(TMP, lhs_low, AT);
        ...
        switch (if_cond) {
          case kCondEQ:
            if (IsUint<16>(rhs_imm)) {
              __ Xori(TMP, lhs, rhs_imm);
              __ Beqz(TMP, true_target);
            } else {
              __ LoadConst32(TMP, rhs_imm);
              __ Beq(lhs, TMP, true_target);
            }
            break;
          case kCondNE:
            if (IsUint<16>(rhs_imm)) {
              __ Xori(TMP, lhs, rhs_imm);
              __ Bnez(TMP, true_target);
            } else {
              __ LoadConst32(TMP, rhs_imm);
              __ Bne(lhs, TMP, true_target);
            }
            break;
          case kCondLT:
            if (IsInt<16>(rhs_imm)) {
...
void CodeGeneratorMIPS::GenerateStaticOrDirectCall(HInvokeStaticOrDirect* invoke, Register temp) {
  ...
  if (invoke->IsStringInit()) {
    ...
    // temp = method;
    __ Lw(temp, SP, kCurrentMethodStackOffset);
...
void InstructionCodeGeneratorMIPS::VisitLoadClass(HLoadClass* cls) {
  ...
  if (cls->IsReferrersClass()) {
    ...
    __ Lw(out, SP, kCurrentMethodStackOffset);
    __ LoadFromOffset(kLoadWord, out, out,
