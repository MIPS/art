    ...
    if (kUseBakerReadBarrier) {
      ...
      if (kBakerReadBarrierThunksEnableForGcRoots) {
        ...
        const int32_t entry_point_offset =
            CodeGenerator::GetReadBarrierMarkEntryPointsOffset<kMipsPointerSize>(0);
...
void LocationsBuilderMIPS::VisitInvokeStaticOrDirect(HInvokeStaticOrDirect* invoke) {
  ...
  bool has_extra_input = invoke->HasPcRelativeDexCache() && !is_r6 && !has_irreducible_loops;
...
void CodeGeneratorMIPS::GenerateStaticOrDirectCall(HInvokeStaticOrDirect* invoke, Location temp) {
  ...
  Register base_reg = (invoke->HasPcRelativeDexCache() && !is_r6 && !has_irreducible_loops)
  ...
  switch (method_load_kind) {
    ...
    case HInvokeStaticOrDirect::MethodLoadKind::kDexCachePcRelative:
      if (is_r6 || has_irreducible_loops) {
        uint32_t offset = invoke->GetDexCacheArrayOffset();
        if (!is_r6) {
          offset -= kDexCacheArrayLwOffset;
        }
        CodeGeneratorMIPS::PcRelativePatchInfo* info_high =
        ...
        CodeGeneratorMIPS::PcRelativePatchInfo* info_low =
            NewPcRelativeDexCacheArrayPatch(invoke->GetDexFileForPcRelativeDexCache(),
                                            offset,
                                            info_high);
        EmitPcRelativeAddressPlaceholderHigh(info_high, TMP, ZERO);
        __ Lw(temp.AsRegister<Register>(), TMP, /* placeholder */ 0x5678, &info_low->label);
      } else {
...
void InstructionCodeGeneratorMIPS::VisitMipsDexCacheArraysBase(HMipsDexCacheArraysBase* base) {
  ...
  CodeGeneratorMIPS::PcRelativePatchInfo* info_low =
      codegen_->NewPcRelativeDexCacheArrayPatch(base->GetDexFile(),
                                                base->GetElementOffset(),
                                                info_high);
  CHECK(!codegen_->GetInstructionSetFeatures().IsR6());
  // TODO: Reuse MipsComputeBaseMethodAddress on R2 instead of passing ZERO to force emitting NAL.
  codegen_->EmitPcRelativeAddressPlaceholderHigh(info_high, reg, ZERO);
  __ Addiu(reg, reg, /* placeholder */ 0x5678, &info_low->label);
}
